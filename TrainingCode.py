...
messages = [command]'''
natural_language_embedding = np.loadtxt('/workspace/LE.txt',delimiter=' ')
...
def bridge_map_action(to_step: rlds.Step, from_step: rlds.Step):
  ...
  to_step['action']['terminate_episode'] = terminate_bool_to_act(
       tf.cast(from_step['is_terminal'], dtype=tf.float32)
  )

  to_step['action']['rotation_delta'] = from_step['observation']['vector_data'][3:6]

  open_gripper = tf.cast(from_step['action'][6], dtype=tf.bool)
